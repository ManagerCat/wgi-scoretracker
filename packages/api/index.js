...
async function handleGeocode(req, res) {
  ...
  try {
    ...
    if (data.status === 'OK' && data.results && data.results.length > 0) {
      ...
      // Cache the result
      setCache(geocodeCache, normalizedAddress, geocodeResult, GEOCODE_CACHE_TTL);
      console.log(`Geocoded: ${address} -> ${geocodeResult.lat}, ${geocodeResult.lng}`);
      
      return res.json({ ...geocodeResult, cached: false });
    } else {
      console.warn(`Geocode failed for: ${address}, status: ${data.status}`);
      // Cache null result to avoid repeated failed requests
      setCache(geocodeCache, normalizedAddress, { result: null }, GEOCODE_CACHE_TTL);
      return res.json({ result: null, status: data.status });
    }
  } catch (error) {
    console.error('Geocoding error:', error);
    return res.status(500).json({ error: 'Geocoding request failed', message: error.message });
  }
}

// Main HTTP endpoint
http("apiGet", async (req, res) => {
  // Set CORS headers
  res.set('Access-Control-Allow-Origin', '*');
